...
const Key = enum { m, space, r, f, l, g, q, e, w, s, a, d };
const Ecs = struct {
...
test "real runtime keyboard paths change left only including lamp geometry" {
    ...
    for ([_]Key{ .space, .f, .m, .g, .e, .w, .d }) |key| {
        game.key = key;
    ...
    try std.testing.expect(!ecs.mists[0].enabled and ecs.mists[1].enabled);
