...
pub fn generateMainZigFromTemplate(
    ...
    // ── Lifecycle section (rendered from backend template, same as procedural path) ──
    {
        ...
        if (use_callback_lifecycle) {
            ...
            // Sokol-backend builds also get the GL PBO readback helpers
            // (labelle-assembler#122 slice 1) — module-scope state plus
            // the comptime-gated GL extern namespace. Wasm currently
            // routes through the raylib branch below, so it's left out
            // of the concat here. The helpers compile to nothing on
            // Metal / D3D11 targets thanks to the `_sokol_preview_gl_enabled`
            // gate inside; emit-unconditional for sokol keeps the code
            // path uniform across desktop / mobile templates.
            const sokol_readback_helpers: []const u8 = if (cfg.backend == .sokol) PREVIEW_READBACK_HELPERS_SOKOL else "";
            ...
            if (cfg.backend == .sokol) {
                ...
                // PBO readback hookups (labelle-assembler#122 slice 1):
                //   - init   : stash the allocator into the module-scope slot
                //   - frame  : the comptime-gated readback block (between
                //              flushScene/gui draw and window.endFrame())
                //   - cleanup: endFrameStream + glDeleteBuffers + free the
                //              CPU staging buffer, BEFORE the graceful `bye`
                // All three blocks evaporate on non-GL sokol targets via
                // the `_sokol_preview_gl_enabled` comptime flag.
                const preview_setup_sokol = try std.mem.concat(allocator, u8, &.{ PREVIEW_INIT_CALLBACK, PREVIEW_READBACK_INIT_SOKOL });
                defer allocator.free(preview_setup_sokol);
                const preview_cleanup_sokol = try std.mem.concat(allocator, u8, &.{ PREVIEW_READBACK_CLEANUP_SOKOL, PREVIEW_CLEANUP_CALLBACK });
                ...
                try tpl.render(lifecycle_tmpl, .{
                    ...
                    .preview_readback = PREVIEW_READBACK_FRAME_SOKOL,
