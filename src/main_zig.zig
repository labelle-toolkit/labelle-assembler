...
// Loop-lifecycle code builders extracted to `src/codegen/lifecycle/loop.zig`
// (refactor #183 step 8a). Re-exported so `generateMainZigFromTemplate`'s
// internal call sites — `buildSetupCode(...)` / `buildGuiDrawCode(...)` —
// stay unchanged.
pub const buildSetupCode = lifecycle_loop.buildSetupCode;
pub const buildGuiDrawCode = lifecycle_loop.buildGuiDrawCode;

// ── Re-exports from src/codegen/lifecycle/callback.zig (refactor #183 step 8b) ─
//
// Callback-lifecycle builders for sokol / wasm / Android backends moved
// to a dedicated submodule. The orchestrator below still calls them
// through the bare names below, so no orchestrator-site change. The
// submodule reaches the still-here helpers through the `pub const`
// re-exports above (asset_wiring, resource_loader).
...
// `buildSetupCode` lives in `src/codegen/lifecycle/loop.zig` as of the
// #183 step-8a refactor. It is re-exported below alongside the
// `lifecycle_loop` namespace handle so the orchestrator's `buildSetupCode(...)`
// + `buildGuiDrawCode(...)` call sites stay unchanged.
...
// `buildGuiDrawCode` lives in `src/codegen/lifecycle/loop.zig` as of the
// #183 step-8a refactor — see the re-exports near the top of this file.

// ============================================================
// Callback-lifecycle code builders (sokol — init/frame/cleanup callbacks)
// MOVED to `src/codegen/lifecycle/callback.zig` (refactor #183 step 8b).
// See the `pub const lifecycle_callback = ...` re-export near the top
// of this file. The orchestrator below now reaches the three builders
// through the aliases `buildCallbackInitCode` / `buildImmersiveEntryCode`
// / `buildCallbackCleanupCode`.
// ============================================================
