...
/// see `pruneOrphanSidecars`. Kept as a prefix (no em-dash tail) so a
...
pub fn scanAndEmit(
    ...
    // Candidate generated sidecars — every `*.zig` in the flows tree,
    // collected in the same walk so the prune pass below doesn't need a
    // second directory traversal. Same normalised-`/` relative form as
    // `rel_paths`.
    var sidecar_paths: std.ArrayList([]const u8) = .empty;
    defer {
        for (sidecar_paths.items) |p| allocator.free(p);
        sidecar_paths.deinit(allocator);
    }
    ...
    while (try walker.next(io)) |entry| {
        ...
        const is_flow = std.mem.endsWith(u8, entry.basename, flow_ext);
        const is_sidecar_candidate = std.mem.endsWith(u8, entry.basename, ".zig");
        if (!is_flow and !is_sidecar_candidate) continue;
        // `entry.path` may use the host separator on Windows; normalise
        // to `/` so emitted import paths and identifiers stay portable.
        // (Opens/deletes below join these onto native base paths — the
        // same mixed-separator form every other read in this file uses.)
        const rel = try allocator.dupe(u8, entry.path);
        std.mem.replaceScalar(u8, rel, std.fs.path.sep, '/');
        if (is_flow) {
            try rel_paths.append(allocator, rel);
        } else {
            try sidecar_paths.append(allocator, rel);
        }
    }
    const sortRelPaths = struct {
        fn lt(_: void, a: []const u8, b: []const u8) bool {
            return std.mem.order(u8, a, b) == .lt;
        }
    }.lt;
    std.mem.sort([]const u8, rel_paths.items, {}, sortRelPaths);
    // Deterministic prune/log order, mirroring the render order above.
    std.mem.sort([]const u8, sidecar_paths.items, {}, sortRelPaths);

    // Prune stale generated sidecars BEFORE rendering: a removed or
    // renamed flow must not leave its old `<stem>.zig` behind (#632) —
    // stale sidecars carry qualified plugin-event tags that act as
    // phantom consumers for the #630 consumption filter (the scan skips
    // them via the #631 pairing rule, but anything else walking
    // `scripts/flows/` would still trip over them).
    try pruneOrphanSidecars(allocator, src_dir, sidecar_paths.items, rel_paths.items);
...
/// Delete stale generated flow sidecars (#632). A `<stem>.zig` under
/// `scripts/flows/**` is deleted only when BOTH halves of the safety
/// predicate hold:
///
...
///
/// `sidecar_rels` / `source_rels` are `/`-normalised paths relative to
/// `flows_dir`; opens and deletes go through the `flows_dir` handle
/// with those sub-paths, so pruning is separator-robust on Windows.
fn pruneOrphanSidecars(
    allocator: std.mem.Allocator,
    flows_dir: std.Io.Dir,
    sidecar_rels: []const []const u8,
    source_rels: []const []const u8,
) !void {
    const io = config.globalIo();
    for (sidecar_rels) |rel_zig| {
        // Half 1 — pairing. The paired source sits at the sidecar's own
        // path with `.zig` swapped for `.flow.jsonc`.
        const stem = rel_zig[0 .. rel_zig.len - ".zig".len];
        const expected_source = try std.fmt.allocPrint(allocator, "{s}{s}", .{ stem, flow_ext });
        defer allocator.free(expected_source);
        const paired = for (source_rels) |src_rel| {
            if (std.mem.eql(u8, src_rel, expected_source)) break true;
        } else false;
        if (paired) continue;
