...
pub fn Mixin(comptime Self: type) type {
    return struct {
        ...
        pub fn renderLifecycle(
            ...
            // ONLY signal for a built-in callback backend too, now that sokol/bgfx
            // declare their shape (the former per-backend enum terms are gone,
            // #461). An UNDECLARED callback external stays rejected — the
            // fail-fast is the safety property.
            ...
            if (use_callback_lifecycle) {
                ...
                // Every callback block is selected by DATA — the manifest
                // `.platforms.<p>.lifecycle` declaration (`lifecycle_ovr`) — never
                // a `cfg.backend` enum branch: a built-in sokol/bgfx manifest
                // declares its FULL shape (including the privileged `preview =
                // .sokol_readback` / `android_register = .bgfx_shell`), and an
                // unprivileged third-party callback backend declares only the safe
                // flags (its preview/android default to none). The privileged
                // values are namespace-gated at manifest validation
                // (`backend_registry.assertLifecyclePrivilege`), so a non-`labelle.*`
                // provider can never reach the backend-private readback/shell
                // blocks here. Only the first-party WASM fall-through (no manifest
                // `.lifecycle` — the generic emscripten `emscripten_set_main_loop`
                // else-path) has a hard-coded shape: module-scope runner,
                // INIT/HEARTBEAT preview callbacks, no backend-specific holes.
                // Every callback hole below is a value keyed on this shape,
                // rendered through ONE `tpl.render`.
