...
pub fn Mixin(comptime Self: type) type {
    return struct {
        ...
        pub fn writePluginControllersBlock(self: *Self, bw: anytype) !void {
            ...
            try bw.writeAll("        }\n");
            try bw.writeAll("    }\n\n");
            try bw.writeAll("    /// Call Controller.deinit on every plugin that declares one.\n");
            try bw.writeAll("    /// Mirrors setup(). Skips plugins without a Controller export or without a\n");
            try bw.writeAll("    /// deinit. Arity-dispatched at comptime (labelle-assembler#593): module-\n");
            try bw.writeAll("    /// singleton controllers with no per-game state declare `deinit()` — e.g.\n");
            try bw.writeAll("    /// labelle-scripting's process-wide VM — while stateful ones keep\n");
            try bw.writeAll("    /// `deinit(game)`.\n");
            try bw.writeAll("    pub fn deinit(game: anytype) void {\n");
            try bw.writeAll("        inline for (_plugin_mods) |mod| {\n");
            try bw.writeAll("            if (@hasDecl(mod, \"Controller\")) {\n");
            try bw.writeAll("                const C = @field(mod, \"Controller\");\n");
            try bw.writeAll("                if (@hasDecl(C, \"deinit\")) {\n");
            try bw.writeAll("                    if (comptime @typeInfo(@TypeOf(C.deinit)).@\"fn\".params.len == 0) C.deinit() else C.deinit(game);\n");
            try bw.writeAll("                }\n");
            try bw.writeAll("            }\n");
            try bw.writeAll("        }\n");
            try bw.writeAll("    }\n");
