//! bgfx backend build hook (manifest-v2, epic #453 item 3, PR 10) — the DEDICATED
...
//!
//! ## Scope — bgfx is DESKTOP + ANDROID; only ANDROID is hook-bearing
//!
...
//! bgfx ships NO ios/wasm templates, so this hook has NO ios/wasm arms.
//!
...
//!     reproducing the enum path's `header_android` target-resolution block
//!     (`build_zig.txt:690`-`719`). Names no backend, so it matches sokol's
//!     android resolver exactly (design §4 — the resolution logic is
//!     backend-agnostic).
...
//!     `libc.txt` generation (reproduces `build_zig.txt:883`,`:905`-`916`). Unlike
//!     sokol, the bgfx C++ TUs (bgfx/bx/bimg) are NDK-sysroot-wired by the backend's
//!     OWN build.zig, so this hook does NOT `addSystemIncludePath` on the `bgfx`
//!     artifact — the residual is only the .so's per-API library path + libc.txt.
//!     The generic parts — `linkLibrary(bgfx)`, `linkSystemLibrary(...)`,
//!     `link_libc`, root-module `.pic` — are emitted declaratively by the assembler
//!     from the manifest, NOT here.
//!
...
//! "assembler imports the hook into the generated root package" (§3). Because the
//! whole v2 route is gated-dark (opt-in via the assembler's `backend_manifest_name`,
//! never set on the production `generate` path — §6), PR 10 exercises it only
//! through the golden-cell + hook gates, not a production android build.
...
/// the hook needs no assembler import. bgfx supports only desktop + android; the
/// ios/wasm variants exist for `HookContext` shape-compatibility with `manifest_v2`
/// and are never reached (bgfx declares no ios/wasm platform entry).
...
/// x86_64 on Intel); otherwise arm64. Backend-agnostic — identical to sokol's.
...
/// PURE NDK triple mapping (`build_zig.txt:722`-`726`). The `usr/lib/<triple>/<api>`
/// NDK path is keyed by this.
...
/// Runs before any `b.dependency`, so it constructs no graph nodes. bgfx has no
/// ios/wasm resolved target, so those platforms never reach this.
...
pub const HookContext = struct {
    ...
    /// an editor-preview generation. Mirrors `manifest_v2.HookContext`.
...
/// PURE libc.txt body builder (`build_zig.txt:907`-`914`). Zig does not bundle
...
/// `header_android` (`build_zig.txt:632`-`676`) so the residual behaves identically
/// — env lookups go through `b.graph.environ_map`, FS probes through
...
/// generated main binds `engine.editor_api`.
///
/// Contract v1.1 appends `_editor_set_state` (the engine-side digest gains
/// `"state"`); appended LAST so the v1.0 prefix stays stable. Mirrors the
/// shipped labelle-bgfx hook (0.6.3).
...
pub fn post_wire(b: *std.Build, ctx: HookContext) void {
    switch (ctx.platform) {
        ...
        .android => {
            ...
            // (`build_zig.txt:883`,`:905`-`916`). The bgfx artifact's C++ TUs are
            // already NDK-wired by the backend build.zig, so — unlike sokol — no
            // `addSystemIncludePath` on the artifact here.
...
// ALSO typechecks `resolve_target`/`post_wire` against the real `std.Build` API — a
...
test "editor-preview defaults OFF: a pre-preview HookContext/EmLinkOptions literal still coerces" {
    ...
    // non-preview build) omit `.editor_preview` — the default must be false
    // in BOTH structs so those literals keep compiling and the exports stay
    // off the link line (emcc errors on missing exported symbols otherwise).
