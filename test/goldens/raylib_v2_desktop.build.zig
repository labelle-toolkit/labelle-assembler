...
pub fn build(b: *std.Build) void {
    ...
    test_root.root_module.linkLibrary(raylib);

    switch (target.result.os.tag) {
        .macos => {
            test_root.root_module.linkFramework("OpenGL", .{});
        },
        .linux => {
            test_root.root_module.linkSystemLibrary("GL", .{});
        },
        .windows => {
            test_root.root_module.linkSystemLibrary("opengl32", .{});
        },
        else => {},
    }
