...
pub fn build(b: *std.Build) void {
    ...
    const materials = @import("material_build.zig").create(b, target, optimize, core, &.{ .{ .name = "fog", .json = @embedFile("materials/fog/material.json") }, .{ .name = "water", .json = @embedFile("materials/water/material.json") }, .{ .name = "lamp", .json = @embedFile("materials/lamp/material.json") } }, "desktop", "120", "142");
