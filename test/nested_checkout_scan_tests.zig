...
/// The flow passes walked `scripts/flows/**` with `std.Io.Dir.walk`, which
/// has no prune hook — so a nested checkout parked there was walked in
/// full. `pruneStaleSidecars` is the alarming half: it DELETES what it
/// collects, so the gap meant deleting files out of another branch's
/// checkout, in a tree that has nothing to do with this build (#692).
pub const FLOW_SCANNER_PRUNING = struct {
    const generated_header = "//! Generated by labelle-gui codegen — DO NOT EDIT.\n";

    test "pruneStaleSidecars does not delete inside a nested checkout (#692)" {
        const allocator = std.testing.allocator;
        var tmp = std.testing.tmpDir(.{});
        defer tmp.cleanup();

        // A real orphan sidecar in the project's own flows dir — the
        // negative control: this one MUST still be pruned.
        try writeSample(tmp.dir, "proj/scripts/flows/gone.zig", generated_header);
        // A worktree parked under flows/, holding an equally "orphaned"
        // generated sidecar that belongs to a different revision.
        try writeSample(tmp.dir, "proj/scripts/flows/wt/.git", "gitdir: /elsewhere/.git/worktrees/wt\n");
        try writeSample(tmp.dir, "proj/scripts/flows/wt/theirs.zig", generated_header);

        const game_dir = try tmp.dir.realPathFileAlloc(io, "proj", allocator);
        defer allocator.free(game_dir);
        try generator.flow_scanner.pruneStaleSidecars(allocator, game_dir);

        // The other branch's file is still there.
        tmp.dir.access(io, "proj/scripts/flows/wt/theirs.zig", .{}) catch {
            std.debug.print("pruneStaleSidecars deleted a file inside a nested checkout\n", .{});
            return error.NestedCheckoutFileWasDeleted;
        };
        // …and the project's own orphan was still pruned.
        try std.testing.expectError(
            error.FileNotFound,
            tmp.dir.access(io, "proj/scripts/flows/gone.zig", .{}),
        );
    }

    test "pruneStaleSidecars still descends an ordinary subdir (#692 negative control)" {
        const allocator = std.testing.allocator;
        var tmp = std.testing.tmpDir(.{});
        defer tmp.cleanup();

        // A plain folder that merely LOOKS like a worktree parent — the
        // prune keys on the `.git` marker, never on the name.
        try writeSample(tmp.dir, "proj/scripts/flows/worktrees/gone.zig", generated_header);
        ...
        try std.testing.expectError(
            error.FileNotFound,
            tmp.dir.access(io, "proj/scripts/flows/worktrees/gone.zig", .{}),
        );
    }
};
